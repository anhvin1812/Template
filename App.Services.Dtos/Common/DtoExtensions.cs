...
namespace App.Services.Dtos.Common
{
    public static class DtoExtensions
    {
        ...
        public static TListResult ToListResult<TDto, TItemResult, TListResult>(this IEnumerable<TDto> source, int parentId, Func<TDto, IEnumerable<EntityAction>> getDtoActions = null, Func<int, IEnumerable<TItemResult>, IEnumerable<EntityAction>> getListActions = null)
            where TDto : DtoBase
            where TItemResult : ItemResult<TDto>, new()
            where TListResult : ListResult<TDto, TItemResult>, new()
        {
            var itemResults = ToItemResults<TDto, TItemResult>(source, getDtoActions).ToList();
            var listResult = new TListResult { Data = itemResults, EntityActions = getListActions != null ? getListActions(parentId, itemResults) : new List<EntityAction>() };
            return listResult;
        }
